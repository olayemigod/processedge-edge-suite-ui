...
Use for large, high-use or low-data-sensitive reports.
...
- Large/high-use reports should use server filtering and pagination.
...
VetEdge PR #47 should consume this standard progressively using the implementations already present after PR #36:

- Stock Expiry Monitor as the canonical high-volume paginated reference;
- Planned Treatment as a purpose-built paginated provider reference;
- VetEdge Report Center as the generic Query Report host to evolve into a shared-provider-aware host.
...
Those belong to the subsequent shared Export/Print phase after the provider contract is validated by at least two different product/report shapes.
