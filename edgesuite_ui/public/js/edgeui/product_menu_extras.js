...
function itemIsCurrent(item, target) {
  ...
  return Boolean(linkTo && parts.length && (parts.includes(linkTo) || parts.join("/").endsWith(`/${linkTo}`)));
}
...
function installDensityControl(runtime, target) {
  ...
  for (const [value, label] of [["compact", "Compact"], ["comfortable", "Comfortable"], ["touch", "Touch"]]) {
